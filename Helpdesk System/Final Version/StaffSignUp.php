...
?>	
	...
	<table>
				<tr>
					<th colspan="2">
					<h1 class="login-hearder" style="text-align: left">Staff Registration</h1> <br>		
					</th>
				</tr>
				<tr>
					<td>
...
## query database
	...
	$exists = 0;
	$result = $mysqli->query("SELECT staff from staffs WHERE staff = '{$user}' LIMIT 1");
	if ($result->num_rows == 1) {
		$exists = 1;
		$result = $mysqli->query("SELECT email from staffs WHERE email = '{$email}' LIMIT 1");
	...
	if ($exists == 1) echo "<p>Staff name already exists!</p>";
	else if ($exists == 2) echo "<p>Staff name and Email already exists!</p>";
	else if ($exists == 3) echo "<p>Email already exists!</p>";
	else if ($exists == 4) echo "<p>Password not Match</p>";
	else {
		# insert data into mysql database
		$sql = "INSERT  INTO `staffs` (`staff_id`, `staff`, `password`, `fname`, `lname`, `gender`, `email`, `mobile`, `dob`, `nationality`, `add1`, `add2`, `add3`) 
		...
		if ($mysqli->query($sql)) {
			...
			$_SESSION['user']= $user;	
			header( "Refresh:3; url=staffhome.php", true, 303);
